...
import { Check, Sparkles, ArrowLeftRight, X, Clock } from "lucide-react";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";
import { repHours, type Employee, type TimeOffMap } from "@/lib/roster";
import { choresForDate, choresForPerson, suggestChoreSlot, SLOT_LABEL, type ChoreItem, type DayChores } from "@/lib/chores";

const CITA_MIN = 20;

type ApptLite = { employee: string | null; timeMins: number; cancelled: boolean };
type Slot = { startMin: number; endMin: number } | null;
...
function metaText(item: ChoreItem, sug: Slot): string {
  let s = `${SLOT_LABEL[item.slot]} · ${item.minutes} min`;
  if (sug) s += ` · ${fmtTime(sug.startMin)}–${fmtTime(sug.endMin)}`;
  return s;
}
...
export type ChoreState = {
  ...
  strip: ChoreItem[];
  suggestions: Record<string, Slot>;
...
export function useDailyChores(
  activeDate: string, employees: Employee[], timeOff: TimeOffMap, meName: string, appts: ApptLite[],
): ChoreState {
  ...
  const suggestions = useMemo(() => {
    ...
    const items = [...Object.values(day.columns).flat(), ...day.strip];
    for (const it of items) {
      const emp = empByName[it.person];
      if (!emp) { map[it.key] = null; continue; }
      const h = repHours(emp);
      const sched = emp.schedule[weekday];
      const busy = appts
        .filter((a) => a.employee === it.person && !a.cancelled)
        .map((a) => ({ s: a.timeMins, e: a.timeMins + CITA_MIN }));
      map[it.key] = suggestChoreSlot(it, {
        startMin: h.startMin, endMin: h.endMin,
        lunchStartMin: sched?.lunchStartMin ?? null, lunchMinutes: sched?.lunchMinutes ?? 0, busy,
      });
    }
    return map;
  }, [day, employees, appts, activeDate]);
  ...
  return { columns: day.columns, strip: day.strip, suggestions, isDone, toggle, forPerson };
}
...
type SugMap = Record<string, Slot>;

export function ChoreChips({ items, isDone, toggle, color, suggestions }: {
  items: ChoreItem[]; isDone: (k: string) => boolean; toggle: (i: ChoreItem) => void; color?: string; suggestions: SugMap;
}) {
        ...
        return (
          <button key={it.key} onClick={() => toggle(it)} title={it.fullLabel}
            ...
            <span className="flex-1 leading-tight">
              ...
              <span className="text-[9px] text-muted-foreground">{metaText(it, suggestions[it.key])}{note ? ` · ${note}` : ""}</span>
            </span>
          </button>
        );
      })}
...
export function ChoreStrip({ strip, isDone, toggle, suggestions }: {
  strip: ChoreItem[]; isDone: (k: string) => boolean; toggle: (i: ChoreItem) => void; suggestions: SugMap;
}) {
...
export function PersonChores({ items, isDone, toggle, canRequest, date, requesterUid, targets, suggestions }: {
  items: ChoreItem[]; isDone: (k: string) => boolean; toggle: (i: ChoreItem) => void;
  canRequest: boolean; date: string; requesterUid: string; targets: string[]; suggestions: SugMap;
}) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setDlg(null);
    } catch (e) { toast.error("No se pudo enviar: " + (e instanceof Error ? e.message : String(e))); }
  ...
  return (
    <div className="border border-border bg-card p-4 mb-4">
      ...
      <div className="flex flex-col gap-2">
        {items.map((it) => {
          const d = isDone(it.key); const note = coverNote(it); const sug = suggestions[it.key];
          return (
            <div key={it.key} className="flex items-center gap-2 px-3 py-2 border" style={{ borderColor: d ? "#3A8769" : (it.flagged ? "#C53A2D" : "hsl(var(--border))"), backgroundColor: d ? "rgba(58,135,105,0.10)" : "transparent" }}>
              <button onClick={() => toggle(it)} className="flex items-center gap-2 flex-1 text-left" title={it.fullLabel}>
                ...
                <span className="leading-tight">
                  ...
                  <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Clock size={9} /> {metaText(it, sug)}{note ? ` · ${note}` : ""}</span>
                </span>
              </button>
      ...
      {dlg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !sending && setDlg(null)}>
          <div className="bg-card border border-border w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-[11px] text-muted-foreground mb-4">{SLOT_LABEL[dlg.slot]} · {dlg.minutes} min · solo por hoy</div>
